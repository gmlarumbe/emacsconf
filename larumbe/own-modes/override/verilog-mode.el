...
(let* ((verilog-type-font-keywords
       ...
       (verilog-ams-keywords
        (eval-when-compile
          (verilog-regexp-opt
           ;; DANGER Modified by Larumbe DANGER
           '(;; Commented since they are not gonna be used for the time being
...
(setq testing/port-connection-regex "\\.\\([0-9a-zA-Z*_-]*\\)[ ]*\\(([^)]*)\\|,\\)")
  (setq verilog-font-lock-keywords-2
        (append verilog-font-lock-keywords-1
                (list
                 ...
                 ;; DANGER: Modified for syntax highlighting experimentation
                 ;; '("\\([A-Za-z][A-Za-z0-9_]*\\)\\s-*(" 1 font-lock-function-name-face)

                 (list testing/port-connection-regex 1 font-lock-doc-face)
  ...
  (setq verilog-font-lock-keywords-3
        (append verilog-font-lock-keywords-2
                ...
                (list
                 ...
                 ;; ;; DANGER: These are regexps inherited from the ones at imenu regexps but simplified since some buffers seemed not to parse them randomly (maybe because of complexity). However, the ones at imenu and `C-M-d' are recognized at regexp-builder
                 '("^[ ]*\\(?1:\\_<\\(?:\\(?:[a-zA-Z_][a-zA-Z0-9$_]*\\)\\|\\)\\_>\\)\\(?:[ \n]\\)*\\(\\(#\\)\\|[a-zA-Z0-9$_]*\\)\\(?:[ \n]\\)*(" (1 font-lock-function-name-face) (2 font-lock-reference-face)) ;Module (+ Instance)
                 '(")[ \n]*\\(?1:\\_<\\(?:\\(?:[a-zA-Z_][a-zA-Z0-9$_]*\\)\\|\\)\\_>\\)\\(?:[ \n]\\)*(" 1 font-lock-reference-face) ; Only Instance
                 ;; The first one includes # and instances for the second expression, but the # is overriden in the other-punctuation present before
                 )
                )))
;; End of DANGER
