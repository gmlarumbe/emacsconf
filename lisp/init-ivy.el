...
(use-package swiper
  ...
  :init
  (defvar mc/cmds-to-run-once '(swiper-mc)) ; Workaround to have available swiper-mc with lazy loading
  :config
...
Use `swiper-isearch' not showing lines as a default or `swiper' if uarg is
provided."
    (interactive "P")
...
(use-package counsel
  ...
  :config
  (setq counsel-find-file-at-point t)    ; Use ffap to open files
  ;; Counsel find file shortcuts:
  ;;   Press "/ TAB" to go to root directory
  ;;   Press "~" to go to $HOME
  ;;   Press "$" to go to some env variable defined path

  (setq counsel-outline-face-style 'org) ; Use same outline faces as org

  ;; Format `counsel-ag' and `counsel-rg' commands from `ag-arguments' and `larumbe/rg-arguments'.
  ;; Assumes that `init-grep.el' has been loaded before.
  ;; - These commands are also used for counsel projectile implementations.
  ...
  ;; `counsel-ag'/ `counsel-rg' by default look for a .git directory and start a search from the directory containing it.
...
(use-package marginalia ; Replace `ivy-rich' for better annotations
  :demand
  :after ivy
  :config
  (marginalia-mode))
...
(use-package orderless ; Matches filtering
  :demand
  :after ivy
  :config ; Ivy config, according to orderless GitHub README
  ;; The config below has a similar effect to setting: (setq ivy-re-builders-alist '((t . ivy--regex-ignore-order)))
  ...
  ;;
  ;;  - Done for `larumbe/search-forward', `larumbe/search-backward' and `larumbe/symbol-at-point'
  ;;
  (setq ivy-re-builders-alist '((t . orderless-ivy-re-builder)))
  (add-to-list 'ivy-highlight-functions-alist '(orderless-ivy-re-builder . orderless-ivy-highlight)))


(use-package ivy-prescient ; Matches sorting
  :demand
  :after ivy
  :init
  (setq ivy-prescient-enable-filtering nil) ; Only sort matches, leave filtering to orderless
  :config
